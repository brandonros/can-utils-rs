...
fn main() {
    // init device
    let device_handle = tactrix_openport::new();
    ...
    let device_thread = std::thread::spawn(move || {
        let mut handler = move |frame: Vec<u8>| {
            ...
            for (peer_addr, stream) in write_streams_map.iter() {
                ...
                println!("sending device from to socket at {:?}", peer_addr);
        ...
        tactrix_openport::recv(&device_handle_ref, &mut handler);
    });
                ...
                loop {
                    ...
                    match msg {
                        Ok(msg) => {
                            ...
                            let data = &msg[4..];
                            tactrix_openport::send_can_frame(
                                &device_handle_ref,
                                arbitration_id,
                                data,
                            );
